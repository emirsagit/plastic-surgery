        ...
        <div class="splide__track">
            <div class="splide__list">
                ...
                <div class="splide__slide m-auto flex justify-center items-end pb-12">
                    <img data-splide-lazy="{{ $image->path }}" alt="{{ $image->alt }}" width="1080" height="1080">
                    <a class="p-2 bg-gray-700 bg-opacity-50 text-gray-200 rounded" 
                    @if ($service=$image->services->where('language', app()->getLocale())->first())
                        ...
                        <p class="font-bold capitalize flex justify-center">{{ $service->title }}<span
                                class="bg-red-500 text-xs rounded px-2 py-1 ml-1">{{ __('Daha Fazla') }}</span></p>
                        @else
                        href="{{ '/' . app()->getLocale() . '/' .   trans('routes.gallery')}}">
                        <p class="font-bold capitalize flex justify-center">{{ __('Galeri Sayfası') }}<span
                                class="bg-red-500 text-xs rounded px-2 py-1 ml-1">{{ __('Daha Fazla') }}</span></p>
                        @endif
                    </a>
                </div>
