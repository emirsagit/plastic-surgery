        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 flex flex-col">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('admin.services.update', $service) }}" method="POST"
                    enctype="multipart/form-data" class="p-6 bg-white border-b border-gray-200">
                    @csrf
                    ...
                    <div class="w-full">
                        ...
                        <select name="language" id="selectedLanguage" onchange="location.reload()"
                            ...
                            value="{{ old('language') }}">
                            <option value="tr" @if (old('language')==="tr" || $service->language === 'tr') selected
                                @endif>Türkçe</option>
                            <option value="en" @if (old('language')==="en" || $service->language === 'en') selected
                                @endif>İngilizce</option>
                        </select>
                    </div>
                    @error('language')
                    ...
                    <div class="w-full mt-4">
                        ...
                        <input name="title"
                            ...
                            value="{{ old('title') ? old('title') : $service->title }}" type="text">
                    </div>
                    @error('title')
                    <div class="text-red-600">{{ $message }}</div>
                    @enderror
                    ...
                    <div class="mt-4 w-full">
                        ...
                        <textarea name="excerpt"
                            class="bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded py-2 px-4 block w-full h-30 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring @error('excerpt') border-red-500 @enderror">{{ old('excerpt') ? old('excerpt') : $service->excerpt }}</textarea>
                    </div>
                    @error('excerpt')
                    <div class="text-red-600">{{ $message }}</div>
                    @enderror
                    ...
                    <div class="mt-4 w-full">
                        ...
                        <textarea name="body" id="editor"
                            class="bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded py-2 px-4 block w-full h-30 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring @error('body') border-red-500 @enderror">{{ old('body') ? old('body') : $service->body }}</textarea>
                    </div>
                    @error('body')
                    <div class="text-red-600">{{ $message }}</div>
                    @enderror
                    ...
                    <div class="w-full mt-4">
                        <label class="block text-gray-600 dark:text-gray-200 text-sm font-medium mb-2">Alt Etiketi (Foto
                            Adı)</label>
                        <input name="alt" value="{{ $service->alt }}"
                    ...
                    <div class="w-full mt-4">
                        ...
                        <input name="video" value="{{ $service->video }}"
                    ...
                    <div class="w-full mt-4">
                        ...
                        <input name="seo_title" value="{{ $service->seo_title }}"
                    ...
                    <div class="w-full mt-4">
                        ...
                        <input name="seo_description" value="{{ $service->seo_description }}"
...
<script>
    function imageData() {
  return {
    previewUrl: "{{ $service->image ? $service->image : '' }}",
    updatePreview() {
      var reader,
